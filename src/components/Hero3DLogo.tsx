...
export function InteractiveMesh({ 
  ...
  useFrame((state, delta) => {
    ...
    // Only grow when pinned — smooth scale-up entrance animation
    const targetScale = (isPinnedRef?.current ?? true) ? 1 : 0
    introScaleRef.current = THREE.MathUtils.damp(introScaleRef.current, targetScale, 3, delta)
    groupRef.current.scale.setScalar(introScaleRef.current * scaleMultiplier)

    // Scroll-driven rotation computed directly from ref (no React state updates in render loop)
    const scrollRot = scrollOffsetRef ? (scrollOffsetRef.current + 1.5) * 0.4 : 0

    // Evident, rich & smooth multi-axis 3D floating + scroll-driven rotation
    const t = state.clock.elapsedTime
    groupRef.current.position.y = Math.sin(t * 1.2) * 0.55
    groupRef.current.rotation.y = Math.sin(t * 0.75) * 0.5 + scrollRot
    groupRef.current.rotation.x = Math.cos(t * 0.85) * 0.2
  ...
  const shape = useMemo(() => {
    ...
    const points0 = c0.map((p) => new THREE.Vector2(p.x, p.y))
    s.moveTo(points0[0].x, points0[0].y)
    s.splineThru(points0)
    ...
    const points1 = c1.map((p) => new THREE.Vector2(p.x, p.y))
    const hole = new THREE.Path()
    hole.moveTo(points1[0].x, points1[0].y)
    hole.splineThru(points1)
    s.holes.push(hole)

    return s
  }, [])

  // Balanced precision: smooth silhouette without GPU freeze
  const geometry = useMemo(() => {
    const geo = new THREE.ExtrudeGeometry(shape, {
      ...
      bevelThickness: 0.12,
      bevelSize: 0.06,
      bevelSegments: 16,   // Safe CPU increase: much better bevel light curve
      curveSegments: 64,   // Preserved: continuous silhouette without overload
      steps: 1
    })
    geo.computeVertexNormals() // Immediately after mesh generation for smooth shading
    return geo
  }, [shape])
        ...
        <Center>
          <mesh scale={isMobile ? 0.75 : 1.2}>
            ...
            <meshPhysicalMaterial
              color="#ffffff"
              metalness={1}
              roughness={0.12}             // Slightly tighter for more liquid-chrome
              clearcoat={1.0}              // Full clearcoat: adds smooth spherical gloss layer
              clearcoatRoughness={0.0}     // Perfect mirror clearcoat: hides polygon bevel edges
              envMapIntensity={3.2}
              reflectivity={1.0}
            />
          </mesh>
